...
var pagetimeline = function(params){
	...
	// setup logger
	var Logger = require( './logger' ), logFile = params.log || '';
...
pagetimeline.prototype = {
	...
	run:function(runstep, callback){
		...
		self.timeoutId = setTimeout( function(){
			...
			self.emit( 'timeout', {
				code:2,
				msg:'program timeout:' + self.programTimeout
			} );
		}, self.programTimeout );
	...
	report:function(){
		...
		// render results
		var Formatter = require( './formatter' ), renderer = new Formatter( this.results, this.format );

		var renderResult = renderer.render();
		this.echo( renderResult );

		// asserts handling
		var failedAsserts = this.results.getFailedAsserts(), failedAssertsCnt = failedAsserts.length;
